...
class CountdownTimer(CTkFrame):
    def __init__(self, master: any, 
        ...
        self.timer_label = CTkLabel(master=self, text=f"Countdown: 00:00:00")
        self.timer_label.grid(row=0, column=0, columnspan=3, pady=(8, 0), sticky="ew")
        
        self.hour_entry = CTkEntry(master=self, width=32, textvariable=self.hour, justify="center")
        self.hour_entry.grid(row=1, column=0, padx=(16, 0), pady=8, sticky="ew")
        
        self.minute_entry = CTkEntry(master=self, width=32, textvariable=self.minute, justify="center")
        self.minute_entry.grid(row=1, column=1, padx=8, pady=8, sticky="ew")
        
        self.second_entry = CTkEntry(master=self, width=32, textvariable=self.second, justify="center")
        self.second_entry.grid(row=1, column=2, padx=(0, 16), pady=8, sticky="ew")
        
        self.submit_button = CTkButton(master=self, width=32, text="Submit", command=lambda: Thread(target=self.countdown()).start())
        self.submit_button.grid(row=2, column=1, pady=(0, 8))
